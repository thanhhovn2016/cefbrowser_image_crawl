...
namespace CefSharp.Example.Handlers
{
    ...
    public class ExampleResourceRequestHandler : ResourceRequestHandler
    {
        ...
        protected override IResponseFilter GetResourceResponseFilter(IWebBrowser chromiumWebBrowser, IBrowser browser, IFrame frame, IRequest request, IResponse response)
        {
            ...
            Debug.WriteLine("Debug Url request({0})", url);
            //if (url.Scheme == CefSharpSchemeHandlerFactory.SchemeName)
            //{
        ...
        {
            var url = new Uri(request.Url);
            Debug.WriteLine("Debug OnResourceLoadComplete({0})", url);
            ...
            if (memoryStream != null)
            {
                ...
                Debug.WriteLine("Debug OnResourceLoadComplete dataLength=({0})", dataLength);
                string cacheimage = "c:\\cefcache\\" +Path.GetFileName(url.ToString());
                if (!File.Exists(cacheimage)) {
                    StreamWriter writer = new StreamWriter(new FileStream(cacheimage, FileMode.OpenOrCreate));
            ...
            if (filter == null)
            {
                Debug.WriteLine("Debug OnResourceLoadComplete filter=null,reques={0}", request.Identifier.ToString());
                return;
            }
            byte[] data = filter.dataAll.ToArray();
            //filter_NotifyData(data);
            string str = System.Text.Encoding.UTF8.GetString(data);
            int getDataPageNum = 0; // page number of the currently obtained data
            ...
            foreach (IPostDataElement item in elements)
            {
                ...
                Debug.WriteLine("Debug OnResourceLoadComplete dataLength=({0})", item.Bytes);
                //postData is the post form data submitted by the page, you can do the query
            }
            */
        }
